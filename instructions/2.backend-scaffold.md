...
4. 编写配置文件 `app/config/settings.py`
   1. 创建文件并填入以下内容（可根据需要调整默认值）：
      ...
      class Settings(BaseSettings):
          ...
          # 在 Docker 环境中，主机名应使用 "postgres" 而不是 "localhost"
          # 用户名和密码必须与 docker-compose.yml 中的配置一致
...
5. 数据库与会话 `app/databases.py`
   ...
   3. 注意：使用相对导入 `from .config.settings` 而不是绝对导入 `from app.config.settings`。
...
6. FastAPI 入口 `app/main.py`
   1. 文件内容参考：
      ...
      ```
   2. 根据需要在 `api/` 中创建各路由文件，并在此处统一挂载。
   3. 注意：
      - 使用相对导入 `from .config.settings` 和 `from .databases`
      - 使用相对导入 `from .api import` 而不是 `from app.api.v1 import`
      - API 路由文件直接放在 `api/` 目录下，不需要 `v1` 子目录
      - 数据库初始化使用 try-except 处理，避免数据库不可用时阻止应用启动
+++++++++++++++++++++++++++++++++++++++++++++++++++
7. Alembic 初始化
